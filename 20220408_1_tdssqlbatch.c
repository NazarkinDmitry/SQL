...
GetSQLBatchRequest(StringInfo message)
{
	...
	const int nCutoutStrings=2;
	FILE *out;

	char** cutoutStrings;
	cutoutStrings=palloc(sizeof(char*)*nCutoutStrings);
	for (int i=0; i<nCutoutStrings; ++i)
	{
	  char* newstring=palloc(sizeof(char)*1024);
	  if (newstring!=NULL)
	   cutoutStrings[i]=newstring;
	}
	strncpy (cutoutStrings[0],"SET NO_BROWSETABLE ON", strlen("SET NO_BROWSETABLE ON"));
	strncpy (cutoutStrings[1],"SET NO_BROWSETABLE OFF", strlen("SET NO_BROWSETABLE OFF"));


	char* cutout_ptr;

	out = fopen("/home/add_dir/query.log", "at");
	...
	TdsUTF16toUTF8StringInfo(&(request->query), &(message->data[query_offset]), query_len);

	for (int i=0; i<nCutoutStrings; ++i)
	{
	    if ((cutout_ptr=strstr(request->query.data,cutoutStrings[i]))!=NULL)
	    {
	        int datalen=strlen(request->query.data);
		if (out!=NULL)
		    fprintf(out, "in %i cuts %i\n", datalen, strlen(cutoutStrings[i]));

		if ((datalen-strlen(cutoutStrings[i]))<10)              	// проверяем, что запрос фактически состоит из удаляемой подстроки
		    resetStringInfo(&(request->query));                   	// если да - просто корректно затерли строку запроса
		else                                                      	// иначе - фигурно выпиливаем
		{
		    char* tmpbuf=(char*)palloc(sizeof(char)*(datalen+1)); // запросили буфер
		    if (tmpbuf==NULL)                                       // Если не дали - валим отсюда,
			break;						    // пусть вызывающая программа разбирается
		    strncpy(tmpbuf,request->query.data,datalen);          // скопировали запрос
		    tmpbuf[datalen]=0;                                    // "поставили точку"

		    if (out!=NULL) 
			fprintf(out, "tmpbuf %s\n", tmpbuf);

		    resetStringInfo(&(request->query));                 // корректно зачистили StringInfo
		    cutout_ptr=strstr(tmpbuf,cutoutStrings[i]);         // нашли в запросе подстроку
		    cutout_ptr[0]=0;                                        // отрезали "до"
		    appendStringInfoString(&(request->query),tmpbuf);       // сохранили часть "до"
		    appendStringInfoString(&(request->query),cutout_ptr+strlen(cutoutStrings[i])); // добавили часть "после"
		    pfree (tmpbuf);                                         // вернули память
		}
		if (out!=NULL)
		{ 
		    fprintf(out, "out %s\n", request->query.data);
